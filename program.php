...
<?php
	...
	if($id) {
			...
			$url = 'http://192.168.200.168/yoga/session/yogaProgramDetail?type=2&programId='.$id.'&debug=1';
			...
			if($data) {
				...
				if($result) {
					...
					$desc = $result['desc'] ? nl2br($result['desc']) : '';
					$logo = $result['cardLogo'] ? $result['cardLogo'] : '';
				}
				else {
					echo '抱歉，您访问的页面不存在!';
					exit;
				...
				<table width="100%">
					<tr>
						<td style="text-align:center;padding-top:1.5em;">
							<img src="<?php echo $logo;?>" class="brower_width">
						</td>
					</tr>
					<tr>
						<td style="text-align:center;">
							<h3 class="brower_width" style="margin-top:20px;color:#333333"><?php echo $title;?></h3>
						</td>
					</tr>
					<tr>
						<td style="padding:1.5em;">
							<span class="brower_width" ><?php echo $desc;?></span>
						</td>
					</tr>
				</table>
			</div>
...
$(document).ready(function(e) {
	...
	//代替alert命令
	//alertFunc('测试测试');
	$('.brower_width').attr("width", $(window).width()*0.9);
