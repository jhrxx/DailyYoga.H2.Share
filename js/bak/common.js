...
//是否是安卓1080P屏
var is1080P=os.android && screen.width==1080 && window.innerHeight>=850 && window.devicePixelRatio==3;
//是否是iphone5
...
$(document).ready(function(e) {
	...
	function addEvent(){//添加事件
		...
		if(videoBtn.length>0){
			//ios8的video标签必须占据全屏宽度，小尺寸按钮点击播放方案无效，故现在IOS和ANDOID一样作为弹窗播放处理，保持用户体验一致
			/*
			if(os.ios && os.iosVer<=7) video_ios();
			else{
		...
		videoBtn.each(function(i) {
			...
			if(vid && vid!=''){
				...
				if(type=='youku') var video=$('<iframe src="http://player.youku.com/embed/'+vid+'" frameborder=0 allowfullscreen></iframe>').appendTo($(this));
					...
					var video=$('<video type="video/mp4" autoplay>').attr({src:vid}).appendTo($(this));
