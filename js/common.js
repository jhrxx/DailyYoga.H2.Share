...
//iphone4不做一屏兼容，内容高度拉伸到与iphone5等高,默认关闭
var scrollIphone4=true;

//-------------------------------------------------------定义当前站点URL，分享功能会调用到
var siteUrl='http://t.buzzreader.cn/public/iphone/';
...
$(document).ready(function(e) {
	...
	var shareBox=$('<aside id="shareBox"><img src="images/common/share.png"></aside>');
	...
	function init(){
		...
		if(scrollIphone4 && isIphone4) {
			articleBox.css({height:'121%'});//把iphone4的body高度增加到iphone5的高度
		}
		else {
			//$(document).on('touchmove',noEvent);//默认禁止页面上下滑动,若要恢复滑动可用 $(document).off('touchmove',noEvent)
			$(document).off('touchmove',noEvent);
		}
		if(btnShare.length>0){
			shareBox.appendTo(htmlBox);
			btnShare.on('touchend',btnShare_click);
		}//end if
	...
	function btnShare_click(e){
		shareBox.show();
		shareBox.one('touchend',pop_close);
	}//end func
...
function weixin_sign(){
	...
	$.getJSON("http://s.weteam.me/jssdk/get_sign?callback=?&key="+wxKey+"&url="+ localUrl, function(data){
...
//--------------------------------公共函数
	...
	function alertFunc(text,callback){
		alertBox.popOn({text:text,callback:callback});
	}//end func
	...
	function checkStr(str,type){
		...
		switch(type){
			case 0:
				var reg= new RegExp(/^1\d{10}$/);//，手机号码验证，是否以13，14,15,18开头的11位数字
			break;
			case 1:
